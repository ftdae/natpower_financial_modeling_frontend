...
const useParameter = () => {
    ...
    dispatch(
      setOperationStartDate(
        parameterInfos?.find((p) => p?.param_index == "basic_project_inputs")
          ?.value.grid_connection_date || "2032-10-31"
      )
    );
  }, [parameterInfos]);
    ...
    dispatch(
      setValuation({
        valuation_date:
          ...
          )?.value.valuation_date || "2030-01-01",
        cost_of_equity:
          parameterInfos?.find(
            (p) => p?.param_index == "valuation_and_returns_inputs@valuation"
          )?.value.cost_of_equity || 0,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setReturnsSettings({
        carryFunActiveSwitch: r?.gp_lp_carry_functionality || 1,
        numberOfStages: r?.number_of_stages || 1,
        carryBasis: r?.carry_basis || 1,
        stageOne: {
          hurdleRate: r?.hurdle_rate_1 || 0,
          gpPortion: r?.gp_portion_after_hurdle_rate_1 || 0,
          gpPortionForCarry: r?.gp_portion_for_carry_1 || 0,
        },
        stageTwo: {
          hurdleRate: r?.hurdle_rate_2 || 0,
          gpPortion: r?.gp_portion_after_hurdle_rate_2 || 0,
          gpPortionForCarry: r?.gp_portion_for_carry_2 || 0,
        },
        discountRate: r?.gp_discount_rate || 0.07,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    ...
    const switchValue = r?.rampSwitch || 0;
    const percentage = switchValue == 1 ? r?.rampPercentage || 0 : 0;
    dispatch(
      setRampRateSettings({
  ...
  useEffect(() => {
    dispatch(
      setLengthOfOperations(
        parameterInfos?.find((p) => p?.param_index == "basic_project_inputs")
          ?.value.length_of_operations || 540
      )
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    dispatch(
      setLengthOfDecommissioning(
        parameterInfos?.find((p) => p?.param_index == "basic_project_inputs")
          ?.value.length_of_decommissioning || 12
      )
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setWorkingCapital({
        debtor_days: r?.debtor_days || 90,
        creditor_days: r?.creditor_days || 90,
      })
    );
  }, [parameterInfos]);
    ...
    dispatch(
      setVat({
        ...
        percentageOfRevenueSubjectToVAT:
          r?.percentage_of_revenue_subject_to_vat || 0,
        percentageOfCostsAndCapexSubjectToVAT:
          r?.percentage_of_costs_and_capex_subject_to_vat || 0,
        monthlyPaymentsOnAccount: r?.monthly_payments_on_account || 0,
      })
    );
  }, [parameterInfos]);
      ...
      setRevenueSetup({
        forecastProviderChoice:
          CHOICE_DATA[PARAM_TYPE.CHOICE.FORECAST_PROVIDER]?.find(
            (p) => p?.id == r?.forecast_provider
          )?.label || "",
        inflation:
          CHOICE_DATA[PARAM_TYPE.CHOICE.INFLATION]?.find(
            (p) => p?.id == r?.revenue_inflation
          )?.label || "",
        baseYear: r?.revenue_inflation_base_year || (2023 as number),
      } as IRevenueSetup)
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setTriadsIncomeSetting({
        switch: r?.triads_income_switch || 0,
        value: isArray(r?.triads_embedded_export_tariffs)
          ? r?.triads_embedded_export_tariffs[regionIndex]
  ...
  useEffect(() => {
    ...
    dispatch(
      setModoRegion(
        CHOICE_DATA[PARAM_TYPE.CHOICE.MODO_REGION][r?.region_for_modo - 1]
          ?.label || ""
      )
    );
  }, [parameterInfos]);
      ...
      setAssumptionsData([
        {
          ...
          data: {
            ...
            inflation:
              INFLATION_LIST[
                (getParameter(`revenue@setup`)?.value[
                  `inflation_for_${model}`
                ] as number) - 1 || 0
              ],
            baseYear: 2023,
            region:
            ...
            tradingStrategy:
              STRATEGY_LIST[
                (getParameter(`revenue@setup`)?.value
                  ?.trading_strategy as number) - 1 || 0
              ],
          },
        },
      ])
  ...
  useEffect(() => {
    ...
    dispatch(
      setSolarRevenueSettings({
        solarSwitch: solarValue?.solar_switch || 0,
        solarTrackingSwitch: solarValue?.solar_tracking || 0,
        solarRetentionRate: isArray(solarValue?.solar_panel_retention_rate)
          ? solarValue?.solar_panel_retention_rate[0]
          : new Array(60).fill(0),
        pvEnergy: solarValue?.pv_energy_production || 0,
        trackingPercentage: solarValue?.extra_solar_tracking_percentage || 0,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    const switchValue = rego?.rego_switch || 0;
    const price = rego?.rego_unit_price || 0;
    dispatch(
      setREGOSettings({
        regoSwich: switchValue,
        regoPrice: price,
        inflationProfile: INFLATION_LIST[rego?.rego_inflation - 1] || "",
        inflationBaseYear: rego?.rego_inflation_year || 2023,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    const cfd = getParameter("revenue@cfd")?.value;
    const switchValue = cfd?.cfd_switch || 0;
    const price = cfd?.cfd_unit_price || 0;
    dispatch(
      setCFDSettings({
        cfdSwich: switchValue,
        cfdPrice: price,
        inflationProfile: INFLATION_LIST[cfd?.cfd_inflation - 1] || "",
        inflationBaseYear: cfd?.cfd_inflation_year || 2023,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    const embedded = getParameter("revenue@embedded_benefits")?.value;
    const switchValue = embedded?.embedded_benefits_switch || 0;
    const price = embedded?.embedded_benefits_unit_price || 0;
    dispatch(
      setEmbeddedSetting({
        embeddedSwich: switchValue,
        embeddedPrice: price,
        inflationProfile:
          INFLATION_LIST[embedded?.embedded_benefits_inflation - 1] || "",
        inflationBaseYear: embedded?.embedded_benefits_inflation_year || 2023,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    ...
    const trading =
      STRATEGY_LIST[
        ((getParameter(`revenue@setup`)?.value?.trading_strategy as number) ||
          1) - 1
      ];
    const region_for_modo =
      CHOICE_DATA[PARAM_TYPE.CHOICE.MODO_REGION][
        (getParameter("revenue@setup")?.value?.region_for_modo as number) - 1 ||
          0
      ]?.label || "";
    if (model == "")
      ...
      const region_for_afry =
        CHOICE_DATA[PARAM_TYPE.CHOICE.AFRY_REGION][
          (getParameter("revenue@setup")?.value?.region_for_afry as number) -
            1 || 0
        ]?.label || "";
    ...
    dispatch(
      setIntercompanySettings({
        inflation_profile_intercompany_expense:
          INFLATION_LIST[r?.inflation_profile - 1] || "",
  ...
  useEffect(() => {
    ...
    dispatch(
      setStartingAssumptionsForBatteries({
        ...
        batteryAvailability: r?.battery_availablity || 0,
  ...
  useEffect(() => {
    ...
    dispatch(
      setBatteryDisposals({
        batteryDisposalSwitch: r?.disposal_switch || 0,
        disposedRetentionRate: r?.diposal_percentage || 0,
        recyclePricePercent: r?.recycle_percentage || 0,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setBatteryEfficiency({
        efficiencySwitch: r?.forecast_switch || 0,
        fixedEfficiency: r?.fixed_battery_efficiency || 0,
        forecastEfficiency: isArray(r?.forecast_battery_efficiency)
          ? r?.forecast_battery_efficiency[0]
          : new Array(53).fill(0),
      })
    );
  }, [parameterInfos]);
    ...
    dispatch(
      setBatteryAugmentation({
        batteryAugmentationSwitch:
          batteryDuration == 2
            ? r?.augmentation_switch_2
            : batteryDuration == 4
            ? r?.augmentation_switch_4
            : batteryDuration == 8
            ? r?.augmentation_switch_8
            : 0,
  ...
  useEffect(() => {
    const r = getParameter("capex@batteries")?.value;
    const supplierChoice = r?.supplier_choice;
    dispatch(
      setBatteryCubes({
        baseYear:
          supplierChoice == 1
            ? r?.byd_latest_year || 2024
            : supplierChoice == 2
            ? r?.byd_july_inputs_kka_year || 2024
            : 2024,
        data: [
          {
            ...
            value:
              batteryDuration == 8
                ? r?.excubes_price_for_8
                : batteryDuration == 4
                ? r?.excubes_price_for_4
                : batteryDuration == 2
                ? r?.excubes_price_for_2
                : 0,
          },
        ],
        value:
  ...
  useEffect(() => {
    ...
    const r = getParameter("other_inputs@inflation_rate_data")?.value
      ?.inflation_index_table || [new Array(300).fill(1)];
  ...
  useEffect(() => {
    ...
    // 	revenueSetup?.forecastProviderChoice?.toLowerCase() || "modo";
    const batteries =
      (getParameter("capex@batteries")?.value?.payment_profile ||
        (1 as number)) - 1;
    // const land =
  ...
  useEffect(() => {
    const r = getParameter("capex@batteries")?.value;
    const forecastSelection = r?.forecast_selection;
    ...
    const inflationProfile =
      forecastSelection == 0
        ? r?.relative_inflation_profile
        : forecastSelection == 1
        ? r?.absolute_inflation_profile
        : 1;
    const baseYear =
      forecastSelection == 0
        ? r?.relative_inflation_year
        : forecastSelection == 1
        ? r?.absolute_inflation_year
        : 2024;
    dispatch(
      setBessCapexForecast({
  ...
  useEffect(() => {
    ...
    //   })
    // );
    dispatch(setBoPCapexPercentage(r?.uplift_on_top_percentage / 100 || 0));
  }, [parameterInfos]);
  useEffect(() => {
    const r = getParameter("capex@batteries")?.value;
    dispatch(
  ...
  useEffect(() => {
    ...
    dispatch(
      setLandRent({
        switch: admin_cost?.land_rent_switch || 0,
        sensitivity: admin_cost?.land_rent_sensitivity || 0,
        sensitivity_magnitude: admin_cost?.land_rent_sensitivity_magnitude || 0,
        annualLandRent: {
          paymentTerms:
            CHOICE_DATA[PARAM_TYPE.CHOICE.LAND_RENT_PAYMENT_TERMS][
              anual_land_rent?.payment_terms
            ]?.label || "",
          landRentBasis:
            CHOICE_DATA[PARAM_TYPE.CHOICE.LAND_RENT_BASIS][
              anual_land_rent?.land_rent_basis - 1
            ]?.label || "",
          annualLandRentPerAcreCharge:
            anual_land_rent?.annual_land_rent_per_acre_charge || 0,
          annualLandRentPerMWCharge:
            anual_land_rent?.annual_land_rent_per_mw_charge || 0,
          portionPayableDuringConstruction:
            anual_land_rent?.portion_payable_during_construction || 0,
          portionPayableDuringOperations:
            anual_land_rent?.portion_payable_during_operations || 0,
          portionPayableDuringDecommissioning:
            anual_land_rent?.portion_payable_during_decommissioning || 0,
        },
        optionChargeOne: {
          landRentBasis:
            CHOICE_DATA[PARAM_TYPE.CHOICE.LAND_RENT_BASIS][
              option_one?.land_rent_basis_for_option_one - 1
            ]?.label || "",
          annualLandRentPerAcreCharge:
            option_one?.annual_land_option_one_rent_per_acre_charge || 0,
          annualLandRentPerMWCharge:
            option_one?.annual_land_option_one_rent_per_mw_charge || 0,
          paymentTerms:
            CHOICE_DATA[PARAM_TYPE.CHOICE.LAND_RENT_PAYMENT_TERMS][
              option_one?.payment_terms_for_option_one
            ]?.label || "",
          startDate: option_one?.option_one_start_date || "2024-10-01",
          endDate: option_one?.option_one_end_date || "2028-06-30",
        },
        optionChargeTwo: {
          landRentBasis:
            CHOICE_DATA[PARAM_TYPE.CHOICE.LAND_RENT_BASIS][
              option_two?.land_rent_basis_for_option_two - 1
            ]?.label || "",
          annualLandRentPerAcreCharge:
            option_two?.annual_land_option_two_rent_per_acre_charge || 0,
          paymentTerms:
            CHOICE_DATA[PARAM_TYPE.CHOICE.LAND_RENT_PAYMENT_TERMS][
              option_two?.payment_terms_for_option_two
            ]?.label || "",
          annualLandRentPerMWCharge:
            option_two?.annual_land_option_two_rent_per_mw_charge || 0,
          startDate: option_two?.option_two_start_date || "2024-10-01",
          endDate: option_two?.option_two_end_date || "2028-06-30",
          landSizeForOptionTwo: option_two?.land_size_for_option_two || 0,
          capacityForOptionTwo: option_two?.capacity_for_option_two || 0,
        },
        substationRent: {
          substationRentAtEndOfLifeSwitch:
            substation_rent?.substation_rent_at_end_of_life_switch,
          substationRentAtEndOfLifeSwitchAtEndOfLifeSwitchToUseInModel:
            substation_rent?.substation_rent_at_end_of_life_switch_to_use_in_model,
          discountRate: substation_rent?.discountRate,
          timeRemainingOnLease:
            substation_rent?.time_remaining_on_release_at_end_of_decommissioning,
          acresForSubstation: substation_rent?.acres_for_substation,
          longTermCPIAssumption: substation_rent?.long_term_cpi_assumption,
        },
        oneTimePayment: {
          paymentAmount: one_time_payment?.payment_amount || 0,
          paymentDate: one_time_payment?.payment_date || "2024-08-08",
        },
        inflation: {
          profile:
            INFLATION_LIST[
  ...
  useEffect(() => {
    dispatch(
      setAssetName(
        getParameter("basic_project_inputs")?.value?.asset_name || "-"
      )
    );
  }, [parameterInfos]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    dispatch(
      setCostOfAdditions({
        land: land?.land_cost / 1000 || 0,
        poolingSubstation: pooling_substation?.pooling_substation_cost || 0,
        transformers: transformers?.transformers_cost || 0,
        balanceOfPlant: [
          {
            duration: 8,
            value:
              (bop?.aggregated_cost_for_8 || 0) * 1 +
              (bop?.aggregated_cost_for_infra_8 || 0) * 1 +
              (bop?.grid_connection_8 || 0) * 1 +
              (bop?.internal_and_control_connections_8 || 0) * 1 +
              (bop?.power_electronics_8 || 0) * 1 +
              (bop?.writing_and_conduits_8 || 0) * 1 +
              (bop?.dc_cabling_8 || 0) * 1 +
              (bop?.inverter_8 || 0) * 1 +
              (bop?.switch_gear_8 || 0) * 1 +
              (bop?.energy_management_system_8 || 0) * 1 +
              (bop?.monitors_8 || 0) * 1 +
              (bop?.aggregated_cost_for_generation_8 || 0) * 1 +
              (bop?.plant_construction_8 || 0) * 1 +
              (bop?.power_plant_equipment_8 || 0) * 1 +
              (bop?.thermal_8 || 0) * 1 +
              (bop?.fire_8 || 0) * 1 +
              (bop?.battery_racking_8 || 0) * 1 +
              (bop?.foundation_8 || 0) * 1 +
              (bop?.inverter_housing_8 || 0) * 1 +
              (bop?.aggregated_cost_for_installation_8 || 0) * 1 +
              (bop?.enabling_works_8 || 0) * 1 +
              (bop?.distributable_labor_8 || 0) * 1 +
              (bop?.engineering_8 || 0) * 1 +
              (bop?.start_up_8 || 0) * 1 +
              (bop?.legal_costs_8 || 0) * 1 +
              (bop?.construction_insurance_8 || 0) * 1 +
              (bop?.consulting_and_advisory_8 || 0) * 1 +
              (bop?.meters_8 || 0) * 1 +
              (bop?.power_during_cons_8 || 0) * 1 +
              (bop?.software_8 || 0) * 1 +
              (bop?.water_8 || 0) * 1 +
              (bop?.contingency_8 || 0) * 1,
          },
          {
            duration: 2,
            value:
              (bop?.aggregated_cost_for_2 || 0) * 1 +
              (bop?.aggregated_cost_for_infra_2 || 0) * 1 +
              (bop?.grid_connection_2 || 0) * 1 +
              (bop?.internal_and_control_connections_2 || 0) * 1 +
              (bop?.power_electronics_2 || 0) * 1 +
              (bop?.writing_and_conduits_2 || 0) * 1 +
              (bop?.dc_cabling_2 || 0) * 1 +
              (bop?.inverter_2 || 0) * 1 +
              (bop?.switch_gear_2 || 0) * 1 +
              (bop?.energy_management_system_2 || 0) * 1 +
              (bop?.monitors_2 || 0) * 1 +
              (bop?.aggregated_cost_for_generation_2 || 0) * 1 +
              (bop?.plant_construction_2 || 0) * 1 +
              (bop?.power_plant_equipment_2 || 0) * 1 +
              (bop?.thermal_2 || 0) * 1 +
              (bop?.fire_2 || 0) * 1 +
              (bop?.battery_racking_2 || 0) * 1 +
              (bop?.foundation_2 || 0) * 1 +
              (bop?.inverter_housing_2 || 0) * 1 +
              (bop?.aggregated_cost_for_installation_2 || 0) * 1 +
              (bop?.enabling_works_2 || 0) * 1 +
              (bop?.distributable_labor_2 || 0) * 1 +
              (bop?.engineering_2 || 0) * 1 +
              (bop?.start_up_2 || 0) * 1 +
              (bop?.legal_costs_2 || 0) * 1 +
              (bop?.construction_insurance_2 || 0) * 1 +
              (bop?.consulting_and_advisory_2 || 0) * 1 +
              (bop?.meters_2 || 0) * 1 +
              (bop?.power_during_cons_2 || 0) * 1 +
              (bop?.software_2 || 0) * 1 +
              (bop?.water_2 || 0) * 1 +
              (bop?.contingency_2 || 0) * 1,
          },
          {
            duration: 4,
            value:
              (bop?.aggregated_cost_for_4 || 0) * 1 +
              (bop?.aggregated_cost_for_infra_4 || 0) * 1 +
              (bop?.grid_connection_4 || 0) * 1 +
              (bop?.internal_and_control_connections_4 || 0) * 1 +
              (bop?.power_electronics_4 || 0) * 1 +
              (bop?.writing_and_conduits_4 || 0) * 1 +
              (bop?.dc_cabling_4 || 0) * 1 +
              (bop?.inverter_4 || 0) * 1 +
              (bop?.switch_gear_4 || 0) * 1 +
              (bop?.energy_management_system_4 || 0) * 1 +
              (bop?.monitors_4 || 0) * 1 +
              (bop?.aggregated_cost_for_generation_4 || 0) * 1 +
              (bop?.plant_construction_4 || 0) * 1 +
              (bop?.power_plant_equipment_4 || 0) * 1 +
              (bop?.thermal_4 || 0) * 1 +
              (bop?.fire_4 || 0) * 1 +
              (bop?.battery_racking_4 || 0) * 1 +
              (bop?.foundation_4 || 0) * 1 +
              (bop?.inverter_housing_4 || 0) * 1 +
              (bop?.aggregated_cost_for_installation_4 || 0) * 1 +
              (bop?.enabling_works_4 || 0) * 1 +
              (bop?.distributable_labor_4 || 0) * 1 +
              (bop?.engineering_4 || 0) * 1 +
              (bop?.start_up_4 || 0) * 1 +
              (bop?.legal_costs_4 || 0) * 1 +
              (bop?.construction_insurance_4 || 0) * 1 +
              (bop?.consulting_and_advisory_4 || 0) * 1 +
              (bop?.meters_4 || 0) * 1 +
              (bop?.power_during_cons_4 || 0) * 1 +
              (bop?.software_4 || 0) * 1 +
              (bop?.water_4 || 0) * 1 +
              (bop?.contingency_4 || 0) * 1,
          },
        ],
        enterPriseValue: vv,
        substaionBuildDev: substationBuildDev?.substation_build_dev_cost || 0,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setAuxilliaryLossesSettings({
        inflationProfile:
          INFLATION_LIST[(r?.auxiliary_losses_inflation_profile || 1) - 1],
        baseYear: r?.auxiliary_losses_inflation_base_year,
        lossFactor: [
          {
            duration: 2,
            auxiliaryLossValue: r?.auxiliary_losses_factor_2 as number,
          },
          {
            duration: 4,
            auxiliaryLossValue: r?.auxiliary_losses_factor_4 as number,
          },
          {
            duration: 8,
            auxiliaryLossValue: r?.auxiliary_losses_factor_8 as number,
          },
        ],
      })
    );
  ...
  useEffect(() => {
    ...
    const averagePrice =
      revenueSetup?.forecastProviderChoice == "Modo"
        ? r?.average_wholesale_day_ahead_price_for_modo?.[0] ||
          new Array(300).fill(0)
        : revenueSetup?.forecastProviderChoice == "Afry"
        ? annualIndexToMonths(
            r?.average_wholesale_day_ahead_price_for_afry?.[0] ||
              new Array(60).fill(0)
          )
        : new Array(300).fill(0);
  ...
  useEffect(() => {
    ...
    // Safely check if r and r.average_wholesale_day_ahead_price are defined
    const value = r?.residual_value || 0;
        ...
        data: [
          {
            ...
            data: {
              ...
              inflationProfile:
                fixedData?.inflation_profile_1 || "No inflation",
              baseYear: fixedData?.inflation_base_year_1,
            },
          },
          {
            ...
            data: {
              ...
              inflationProfile:
                fixedData?.inflation_profile_2 || "No inflation",
              baseYear: fixedData?.inflation_base_year_2,
            },
          },
        ],
  ...
  useEffect(() => {
    ...
    dispatch(
      setAssetManagement({
        firstPeriod: {
          inflationProfile:
            INFLATION_LIST[r?.inflation_profile_1 - 1 || 0] || "",
          ...
          feesAsAPercentOfRevenue: {
            realTimeManagement:
              r?.real_time_management_percentage_period_1 || 0,
            maintenance: r?.maintenance_percentage_period_1 || 0,
          },
          feesPerMW: {
            realTimeManagement: r?.real_time_management_period_1 || 0,
            maintenance: r?.maintenance_period_1 || 0,
          },
        },
        secondPeriod: {
          inflationProfile:
            INFLATION_LIST[r?.inflation_profile_2 - 1 || 0] || "",
          baseYear: r?.inflation_base_year_2 || 2023,
          startDate: r?.start_date_period_2,
          endDate: r?.end_date_period_2,
          feesAsAPercentOfRevenue: {
            realTimeManagement:
              r?.real_time_management_percentage_period_2 || 0,
            maintenance: r?.maintenance_percentage_period_2 || 0,
          },
          feesPerMW: {
            realTimeManagement: r?.real_time_management_period_2 || 0,
            maintenance: r?.maintenance_period_2 || 0,
          },
        },
      })
    );
      ...
      setOperationAndManagementSettings([
        {
          ...
          baseYear: fixed?.inflation_base_year || 2024,
          cost: [
            { duration: 2, value: fixed?.annual_fixed_o_and_m_2 || 0 },
            { duration: 4, value: fixed?.annual_fixed_o_and_m_4 || 0 },
            { duration: 8, value: fixed?.annual_fixed_o_and_m_8 || 0 },
          ],
        },
        {
          type: "Variable",
          ...
          baseYear: variable?.inflation_base_year || 2024,
          cost: [
            { duration: 2, value: variable?.variable_o_and_m_2 || 0 },
            { duration: 4, value: variable?.variable_o_and_m_4 || 0 },
            { duration: 8, value: variable?.variable_o_and_m_8 || 0 },
          ],
        },
      ])
    );
  ...
  useEffect(() => {
    ...
    dispatch(setDecommissioningCost(r?.decommissioning_total_cost || 0));
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setCommunityBenefit({
        capitalisationSwitch: r?.capitalise_swtich || 0,
        inflationProfile: INFLATION_LIST[r?.inflation_profile - 1 || 0] || "",
        baseYear: r?.inflation_base_year || 2023,
        annualFixedFundToCommunityBenefit:
          r?.annual_fixed_fund_to_community_benefit || 0,
        annualMWhToCommunityBenefit: r?.annual_mwh_to_community_benefit || 0,
        uelYears: r?.useful_economic_life || 40,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setExtended_warranty({
        extended_warranty_switch: r?.extended_warranty_switch || 0,
        inflation_profile_warranty:
          INFLATION_LIST[r?.inflation_profile - 1 || 0] || "",
        inflation_base_year_warranty: r?.inflation_base_year || 2023,
        length_of_warranty: r?.length_of_warranty || 0,
        annual_fees_per_mw: [
          { duration: 2, fee: r?.annual_fees_per_mw_2 || 0 },
          { duration: 4, fee: r?.annual_fees_per_mw_4 || 0 },
          { duration: 8, fee: r?.annual_fees_per_mw_8 || 0 },
        ],
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setOtherAdministrativeCosts({
        inflationProfile: INFLATION_LIST[r?.inflation_profile - 1 || 0],
        baseYear: r?.inflation_base_year,
        annualAccountingFeesAndAudit: r?.annual_accounting_fees_and_audit || 0,
        annualIT: r?.annual_it || 0,
        annualOtherCost: r?.annual_other_cost || 0,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    const selectedUpside = r?.optimiser_upside_value || 0;
    dispatch(
      setOptimiser({
        switch: r?.optimiser_switch || 0,
        commission: r?.optimiser_commission || 0,
        floor: {
  ...
  useEffect(() => {
    ...
    const enterpriseSwitch =
      getParameter("capex@excluding_batteries@enterprise_value")?.value
        ?.enterprise_value_switch || 0;
    const devexProfilData = r?.devex_profile_data;
    dispatch(
      setDevexSetting({
        devexSwitch: 1 - enterpriseSwitch,
        devexProfile: isArray(devexProfilData)
          ? devexProfilData[r?.profile - 1 || 0]
          : new Array(24).fill(0),
        usefulEL: r?.useful_economic_life_devex || 0,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    dispatch(
      setduosChargesSetting({
        connectionSwitch: r?.distribution_connection || 0,
        dnoRegion: r?.dno || -1,
        meteringPoints: r?.number_of_metering_points || 0,
        inflationProfile: INFLATION_LIST[r?.duos_inflation_profile - 1 || 0],
        baseYear: r?.duos_inflation_base_year || 2023,
        duosData: r?.dnuos_data || [
          new Array(14).fill(0),
          new Array(14).fill(0),
          new Array(14).fill(0),
          new Array(14).fill(0),
        ...
        demandRed:
          100 - (r?.duos_demand_amber || 0) - (r?.duos_demand_green || 0),
        demandAmber: r?.duos_demand_amber || 0,
        demandGreen: r?.duos_demand_green || 0,
        generationRed:
          100 -
          (r?.gduos_generation_amber || 0) -
          (r?.gduos_generation_green || 0),
        generationAmber: r?.gduos_generation_green || 0,
        generationGreen: r?.gduos_generation_green || 0,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    ...
    dispatch(
      settnuosTriadChargeSetting({
        switch: r?.tnuos_charges_unavoidable_switch || 0,
        exportPortion:
          r?.anticipated_export_during_triads_as_a_percent_of_grid_connection ||
          0,
        novPortion: r?.portion_of_triads_expected_november || 0,
        decPortion: r?.portion_of_triads_expected_december || 0,
        janPortion: r?.portion_of_triads_expected_january || 0,
        febPortion: r?.portion_of_triads_expected_february || 0,
        demandTariffData: [
          `Northern Scotland`,
          `Southern Scotland`,
          `Northern`,
          ...
          region: d,
          data: Array.isArray(r?.demand_tariffs)
            ? r?.demand_tariffs[index]
            : new Array(55).fill(0),
        })),
      })
    );
  ...
  useEffect(() => {
    ...
    dispatch(
      setExportChargesOfTNUoS({
        transmissionConnectionSwitch: r?.transmission_connection_switch || 0,
        zone: TNUOS_ZONE_LIST[r?.tnuos_zone - 1 || 0],
        localCircuits: LOCAL_CIRCUITS_ZONE[r?.local_circuits - 1],
        annualLoadFactor: r?.annual_load_factor || 1,
        gridConnectionVoltage: r?.grid_connection_voltage || 1,
        localSubstationType: r?.local_substation_type || 1,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setLocalSubstationTariff(
        r?.local_substation_type_by_voltage_data || binResult
      )
    );
  }, [parameterInfos]);
  useEffect(() => {
    ...
    if (r && r.local_circuits && Array.isArray(r.local_circuits_data)) {
      const localCircuitsIndex = r.local_circuits - 1;
      ...
      const localCircuitsData =
        r.local_circuits_data[localCircuitsIndex] || new Array(60).fill(0);
  ...
  useEffect(() => {
    ...
    const opexSwitch = r?.opex_sensitivity;
    const magnitude = (r?.opex_sensitivity_magnitude || 0) / 100;
    ...
    dispatch(
      setNationalGridSecurities({
        ...
        attributable_security_choice:
          VARIABLE_PROFILE_FOR_ATTRIBUTABLE_COSTS[
            r?.attributable_security_choice - 1 || 0
          ],
        total_attributable_costs: r?.total_attributable_costs || 0,
        annual_wider_cancellation_costs:
          r?.annual_wider_cancellation_costs || 0,
        premium_fee: r?.premium_fee || 0,
        usefulEconomicLife: r?.useful_economic_life || 0,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    const attributableSecurityChoiceData = r?.attributable_security_choice_data;
  ...
  useEffect(() => {
    ...
    dispatch(
      setCorporationTax({
        smallProfitsTaxRate: r?.small_profits_tax_rate || 0,
        mainRateOfTax: r?.main_rate_of_tax || 0,
        profitThresholdForSmallProfits:
          r?.profit_threshold_for_small_profits || 0,
        annualInvestmentAllowance: r?.aia || 0,
        rateForCapitalAllowancesSpecialPool:
          r?.rate_for_capital_allowances_capital_pool || 0,
        rateForCapitalAllowancesMainPool:
          r?.rate_for_capital_allowances_main_pool || 0,
        smallPoolAllowanceThreshold: r?.small_pool_allowances_threshold || 0,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(
      setCapexProvision([
        ...
        {
          capexObject: "Capitalised rent in construction",
          months: r?.capitalised_rent_in_construction || 0,
        },
        {
          capexObject: "Land",
          months: r?.land_purchase || 0,
        },
        {
          capexObject: "Pooling substation",
          months: r?.pooling_substation || 0,
        },
        {
          capexObject: "Transformers",
          months: r?.transformers || 0,
        },
        {
          capexObject: "Balance of Plant",
          months: r?.balance_of_plant,
        },
        {
          capexObject: "Enterprise value - Development fee",
          months: r?.ev_dev_fee || 0,
        },
        {
          capexObject: "Community benefit",
          months: r?.capitalised_communnity_benefit_in_construction || 0,
        },
      ])
    );
  }, [parameterInfos]);
  useEffect(() => {
    ...
    dispatch(
      setCashRequirements({
        cashSwitch: r?.minimum_cash_switch || 0,
        minimumCashBalance: r?.minimum_cash_balance || 0,
        cashRequirementLookForwardRestriction:
          r?.cash_requirement_look_forward_restriction || 0,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    ...
    dispatch(
      setGearingByCapexType({
        bessAugmentation: r?.bess_augmentation || 0,
        bessReplacement1: r?.bess_replacement_1 || 0,
        bessReplacement2: r?.bess_replacement_2 || 0,
        excludingBatteries: r?.gearing_excluding_batteries || 0,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    ...
    dispatch(
      setEquity({
        equitySplitToShareholderLoan: r?.equity_split_to_sharholder_loan || 0,
        equitySplitToShareholderCapital:
          100 - r?.equity_split_to_sharholder_loan || 0,
        shareholderLoanInterest: r?.shareholder_loan_interest || 0,
        shareholderLoanCashSweepPercentage:
          r?.shareholder_loan_cash_sweep_percentage_of_available_cash || 0,
        shareCapitalCashSweepPercentage:
          r?.share_capital_cash_sweep_percentage_of_available_cash || 0,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    ...
    dispatch(
      setDividends({
        dividends_cash_sweep_percent_of_available_cash:
          r?.dividends_cash_sweep_percentage_of_available_cash || 0,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    ...
    dispatch(
      setDevelopmentFeePaymentPercentageProfile({
        ...
        paymentPercentageAtLandSecuredDate: r?.percent_land_secured_date || 0,
        paymentPercentageAtGridSecuredDate: r?.percent_grid_secured_date || 0,
        paymentPercentageAtClosingOfDebtAgreementDate:
          r?.percent_closing_debt_agreement || 0,
        paymentPercentageAtFullyConsented: r?.percent_fully_consented || 0,
        paymentPercentageAtrTB: r?.percent_rtb || 0,
        paymentPercentageAtcOD: r?.percent_cod || 0,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    dispatch(
      setSeniorDebt({
        seniorDebtInterst: r?.senior_debt_interest || 0,
        cashSweepPercentOfAvailableCash:
          r?.cash_sweep_percentage_of_available_cash || 0,
        minimumAllowedDSCRHalfYearly: r?.minimum_allowed_dscr_half_yearly || 0,
        minimumAllowedDSCRAnnual: r?.minimum_allowed_dscr_annual || 0,
        arrangementFeePercentage: r?.arrangement_fee_percentage || 0,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    ...
    dispatch(setOperationYears(r?.length_of_operations / 12 || 45));
  }, [parameterInfos]);
  useEffect(() => {
    const r = getParameter(`revenue@tolling`)?.value;
    dispatch(
      setTollingSetting({
        tollingSwitch: r?.tolling_switch || 0,
        startDate: r?.tolling_start_date || operationStartDate,
        endDate: r?.tolling_end_date || decommissioningEndDate,
        tollingPrice: r?.tolling_price || 0,
        tollingPercentage: r?.tolling_percentage || 0,
      })
    );
  }, [parameterInfos]);
  useEffect(() => {
    const r = getParameter(`revenue@gain_on_disposal_of_batteries`)?.value;
    dispatch(setGainOnDisposalSwitch(r?.switch || 0));
  }, [parameterInfos]);
  useEffect(() => {
    ...
    dispatch(
      setDSAFeeSetting({
        baseTotalDSAFee: setting?.base_total_dsa_fee || 0,
        inflationProfile: setting?.inflation_profile,
        baseYear: setting?.base_year || 2024,
        pdaDate: setting?.pda_date || "2020-01-01",
        hotsDate: setting?.hots_date || "2020-01-01",
        landSecuredDate: basic?.land_secured_date || "2020-01-01",
        gridAppDate: setting?.grid_application_date || "2020-01-01",
        gridFirstOfferDate:
          setting?.grid_first_offer_received_date || "2020-01-01",
        gridSecuredDate: basic?.grid_secured_date || "2020-01-01",
        prePlanDate: setting?.preplanning_date || "2020-01-01",
        consultDate: setting?.consultation_date || "2020-01-01",
        planConsentDate: setting?.planning_consent_date || "2020-01-01",
        preConDischargeDate:
          setting?.pre_construction_discharched_date || "2020-01-01",
        pdaMile: setting?.pda_milestone || 0,
        hotsMile: setting?.hots_milestone || 0,
        landSecuredMile: setting?.land_secured_milestone || 0,
        gridAppMile: setting?.grid_application_milestone || 0,
        gridFirstOfferMile: setting?.grid_first_offer_received_milestone || 0,
        gridSecuredMile: setting?.grid_secured_milestone || 0,
        prePlanMile: setting?.preplanning_milestone || 0,
        consultMile: setting?.consultation_milestone || 0,
        planConsentMile: setting?.planning_consent_milestone || 0,
        preConDischargeMile:
          setting?.pre_construction_discharched_milestone || 0,
      })
    );
  }, [parameterInfos]);
  ...
  useEffect(() => {
    const r = getParameter(`capex@batteries`)?.value;
    const scenario = r?.capex_forecast_scenario_choice;
