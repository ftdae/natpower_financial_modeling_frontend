...
const headers = [
  {
    rowTitle: "Entity",
    columns: [
      { text: "Last year revenue £'000", colSpan: 1 },
      { text: "Last year opex £'000", colSpan: 1 },
      { text: "Last year opex as % of revenue %", colSpan: 1 },
      { text: "Total capex (excluding replacement/refurbishments) £'000", colSpan: 1 },
      { text: "Last year revenue as % of total capex %", colSpan: 1 },
      { text: "Last year opex as % of total capex %", colSpan: 1 },

    ],
  },
];

const AggregatedLastYearFinancialsTable = ({
  dashboardStyle = true,
  ...
  const aggregated_last_year_financials = emarineCalculationResults?.aggregated_last_financials
  let resultArray = [aggregated_last_year_financials];
  ...
  return (
    <div className={`my-6 ${!dashboardStyle ? "mx-auto" : ""} `}>
      <h1 className="text-[2rem] mx-6 font-bold flex items-baseline gap-2">
        ...
        />
        Financials of the last operating year (12 months before the end of operation)
      </h1>
                ...
                </td>
                {
                  <React.Fragment key={0}>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_last_year_revenue || 0, 0))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_last_year_opex || 0, 2))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_last_opex_to_revenue * 100 || 0, 2))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_last_year_total_capex || 0, 2))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_last_revenue_to_capex * 100 || 0, 2))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_last_opex_to_capex * 100 || 0, 2))}
                    </td>
                  </React.Fragment>
                }
              </tr>
...
export default AggregatedLastYearFinancialsTable;
