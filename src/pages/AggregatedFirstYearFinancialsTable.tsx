...
const AggregatedFirstYearFinancialsTable = ({
  dashboardStyle = true,
  ...
  const aggregated_first_year_financials = emarineCalculationResults?.aggregated_first_financials
  let resultArray = [aggregated_first_year_financials];
                ...
                </td>
                {
                  <React.Fragment key={0}>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_first_year_revenue || 0, 0))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_first_year_opex || 0, 2))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_first_opex_to_revenue * 100 || 0, 2))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_first_year_initial_capex || 0, 2))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_first_revenue_to_capex * 100 || 0, 2))}
                    </td>
                    <td className="border border-gray-300 text-center px-4 py-2">
                      {redNumber(roundNumber(dd?.aggregated_first_opex_to_capex * 100 || 0, 2))}
                    </td>
                  </React.Fragment>
                }
              </tr>
...
export default AggregatedFirstYearFinancialsTable;
