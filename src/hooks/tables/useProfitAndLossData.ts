...
export function useProfitLossData(active: string, hasTotal: string) {
  const { currentProjectType, emarineCalculationResults, parameters } =
  ...
  const emarineProfitAfterTax = getFilterData(
    roundArray(
      emarineCalculationResults?.current_project?.pl_results
        ?.profit_after_tax || new Array(periodEmarine).fill(0),
      10
    ),
    modelStartDate,
    active,
  ...
  const emarineTableData = useMemo(() => {
    ...
    let plResults = emarineCalculationResults?.current_project?.pl_results;
    const rlt = {
      id: "profit_and_loss",
      title: "Profit and Loss",
      data: [],
