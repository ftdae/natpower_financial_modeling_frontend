...
export function useAggregatedCashFlowData(active, hasTotal) {
  ...
  const emarineNetCashflow = getFilterData(
    roundArray(
      emarineCalculationResults?.quarterly_aggregated_cf?.net_cashflow ||
  ...
  const emarineTableData = useMemo(() => {
    ...
    const cfResults = emarineCalculationResults?.quarterly_aggregated_cf;
