...
import { DEFAULT_COMMUNITY_BENEFIT } from "./constant";
import { ICommunityBenefit } from "./type";
...
export function calcCommunityBenefit({
  ...
  const forecastCostToPL: number[] = sumArrays(
    communityBenefitCost,
    multiplyNumber(forecastCostToCapex, -1)
  );
  return addZeros(forecastCostToPL, period);
}
