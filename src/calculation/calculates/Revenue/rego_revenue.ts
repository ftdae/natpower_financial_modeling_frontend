...
import { IInflationForm, IRegoSettings, IVintage } from "../Revenue/type";
import {
  DEFAULT_INFLATION_INPUTS,
  DEFAULT_REGO_SETTINGS,
  DEFAULT_VINTAGE,
} from "../constant";
import {
  annualIndexToMonths,
...
export function calcREGORevenue({
  regoSettings = DEFAULT_REGO_SETTINGS,
  ...
  const indexation = normalizeArray(
    annualIndexToMonths(
      inflationIndex({
        inflationInputs,
        baseYear: regoSettings?.inflationBaseYear || 2023,
        profile: regoSettings?.inflationProfile,
  ...
  const forecastRevenue = vintage.totalMWGenerated.map(
    (d, index) =>
      (((d *
        regoSettings.regoSwich *
        regoSettings.regoPrice *
