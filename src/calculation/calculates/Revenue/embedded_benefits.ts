...
import { IEmbeddedSetting, IInflationForm, IVintage } from "../Revenue/type";
import {
  DEFAULT_EMBEDDED_SETTING,
...
export function calcEmbeddedBenefits({
  embeddedSettings = DEFAULT_EMBEDDED_SETTING,
  ...
  embeddedSettings?: IEmbeddedSetting;
  ...
  const indexation = normalizeArray(
    annualIndexToMonths(
      inflationIndex({
        inflationInputs,
        baseYear: embeddedSettings?.inflationBaseYear || 2023,
        profile: embeddedSettings?.inflationProfile,
  ...
  const forecastRevenue = vintage.totalMWGenerated.map(
    (d, index) =>
      (((d *
        embeddedSettings.embeddedSwich *
        embeddedSettings.embeddedSwich *
