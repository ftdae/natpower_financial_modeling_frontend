...
export function calcPoolingSubstationAdditions({
  ...
  // poolingSubstationProfile ~~~ calcs row 3874
  let poolingSubstationProfile: number[] = new Array(11).fill(0);
  poolingSubstationProfile[0] =
    paymentMilestonesData[0] * 1 + paymentMilestonesData[1] * 1;
  for (let i = 1; i < 10; i++) {
    poolingSubstationProfile[i] =
      1 * paymentMilestonesData[i * 3 - 1] +
      1 * paymentMilestonesData[i * 3] +
      1 * paymentMilestonesData[i * 3 + 1];
  }
  poolingSubstationProfile[10] = paymentMilestonesData[29];
  const paymentMilestonesLength = poolingSubstationProfile.length;
  ...
  for (let i = 0; i < paymentMilestonesLength; i++)
    ...
    ] = (additionsCost * poolingSubstationProfile[i]) / 100;
